...
def generer_plan_personnalise(distance_course, D_plus, duree_semaine, sorties_par_semaine):
    ...
    for semaine in range(1, duree_semaine + 1):
        # Progression linéaire avec taper final
...
if uploaded_file is not None:
    ...
    if analyse:
        ...
        # Fond clair et quadrillage noir
        ...
        # --- Paramètres pour le plan d'entraînement ---
        st.subheader("⚙️ Paramètres du plan d'entraînement")
        distance_course = analyse['distance_totale_km']
        D_plus = analyse['D_plus_m']
        duree_semaine = st.slider("Durée du plan (semaines)", 4, 20, 8)
        sorties_par_semaine = st.slider("Nombre de sorties par semaine", 2, 4, 3)
        
        if st.button("Générer le plan d'entraînement"):
            plan_df = generer_plan_personnalise(distance_course, D_plus, duree_semaine, sorties_par_semaine)
